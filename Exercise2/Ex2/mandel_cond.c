...
// Define the condition variable and the mutex
pthread_mutex_t mtx = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t cond = PTHREAD_COND_INITIALIZER;

// Define the line to print
int next_line = 0;
...
void *thread_start_fn(void* arg){
    ...
    for (line = thr->thrid; line < y_chars; line += thr->thrcnt){
        ...
        if (ret != 0) {
            perror_pthread(ret, "Error locking\n");
            exit(1);
        }       
        while(line != next_line){
            int ret = pthread_cond_wait(&cond, &mtx);
            if (ret != 0) {
                perror_pthread(ret, "Error waiting on condition variable\n");
                exit(1);
            }
        }
        ...
        if (ret != 0) {
            perror_pthread(ret, "Error broadcasting condition variable\n");
            exit(1);
        }
        ret = pthread_mutex_unlock(&mtx);
        if (ret != 0) {
            perror_pthread(ret, "Error unlocking mutex\n");
            exit(1);
        }
    }
...
int main(int argc, char * argv[]){
    ...
    if (ret != 0) {
        perror_pthread(ret, "Error destroying mutex\n");
        exit(1);
    }
    ret = pthread_cond_destroy(&cond);
    if (ret != 0) {
        perror_pthread(ret, "Error destroying condition variable\n");
        exit(1);
    }
