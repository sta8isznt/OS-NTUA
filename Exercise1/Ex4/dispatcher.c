...
typedef enum { JOB_PENDING, JOB_IN_PROGRESS, JOB_DONE } job_state_t;
...
static void send_text(int fd,const char *s){ write_all(fd,s,strlen(s)); }
...
static void init_jobs(const char *file,job_t **jobs,size_t *njobs){
    struct stat st;
    size_t i,n;
    if(stat(file,&st)<0){ write_message(2,"stat failed\n"); exit(1); }
    if(st.st_size==0){ *jobs=NULL; *njobs=0; return; }
    n=(size_t)((st.st_size+CHUNK_SIZE-1)/CHUNK_SIZE);
    *jobs=malloc(n*sizeof(job_t));
    if(*jobs==NULL){ write_message(2,"malloc failed\n"); exit(1); }
    *njobs=n;
    for(i=0;i<n;i++){
...
/*
 ...
 If no pending job exists, return -1.
 */
...
/*
 Return the index of the next alive and jobless worker using round-robin .
...
static void clear_worker(worker_t *w){
    if(w->alive){ close(w->to_fd); close(w->from_fd); }
    w->pid=0; w->alive=0; w->busy=0; w->current_job_id=-1;
}
/*
 ...
 Then we fork and exec the worker executable.
...
static void spawn_one_worker(worker_t w[],const char *file,int cmd_fd,int resp_fd){
    ...
    pid_t pid;
    if(slot<0){ send_text(resp_fd,"No free worker slot\n"); return; }
    if(pipe(to_p)<0||pipe(from_p)<0){ write_message(2,"pipe failed\n"); exit(1); }
    pid=fork();
    if(pid<0){ write_message(2,"fork failed\n"); exit(1); }
    if(pid==0){
        char rfd[16],wfd[16];
        char *newargv[]={"./worker",(char *)file,rfd,wfd,NULL};
        for(i=0;i<MAX_WORKERS;i++) if(w[i].alive){ close(w[i].to_fd); close(w[i].from_fd); }
        close(cmd_fd); close(resp_fd);
        close(to_p[1]); close(from_p[0]);
    ...
    close(to_p[0]); close(from_p[1]);
    w[slot].pid=pid; w[slot].to_fd=to_p[1]; w[slot].from_fd=from_p[0];
    w[slot].alive=1; w[slot].busy=0; w[slot].current_job_id=-1;
}
...
static void remove_workers(worker_t w[],int n,int resp_fd){
    message_t msg; char buf[128]; int i;
    memset(&msg,0,sizeof(msg)); msg.type=CMD_TERMINATE_WORKER;
    for(i=MAX_WORKERS-1;i>=0&&n>0;i--) if(w[i].alive){ write_all(w[i].to_fd,&msg,sizeof(msg)); n--; }
    snprintf(buf,sizeof(buf),"Worker removal requested, active workers = %d\n",active_workers(w));
    send_text(resp_fd,buf);
}

static void assign_jobs(worker_t w[],job_t *jobs,size_t njobs,char target){
    while(1){
        int j=find_pending_job(jobs,njobs),k=next_free_worker(w);
        message_t msg;
        if(j<0||k<0) break;
        ...
        if(write_all(w[k].to_fd,&msg,sizeof(msg))!=(ssize_t)sizeof(msg)){ clear_worker(&w[k]); continue; }
        w[k].busy=1; w[k].current_job_id=j;
        jobs[j].state=JOB_IN_PROGRESS; jobs[j].worker_pid=w[k].pid;
    }
}
...
static void send_progress(int resp_fd,job_t *jobs,size_t njobs){
    char buf[128]; int percent=(njobs==0)?100:(int)((done_jobs(jobs,njobs)*100)/njobs);
    snprintf(buf,sizeof(buf),"PROGRESS: %d%% complete, matches so far: %d\n",percent,total_matches(jobs,njobs));
    send_text(resp_fd,buf);
}
...
int main(int argc,char *argv[]){
    ...
    if(argc!=5){ write_message(2,"Usage: ./dispatcher <cmd_fd> <resp_fd> <file> <char>\n"); exit(1); }
    cmd_fd=atoi(argv[1]); resp_fd=atoi(argv[2]); file=argv[3];
    if(argv[4][0]=='\0'||argv[4][1]!='\0'){ write_message(2,"dispatcher needs one character\n"); exit(1); }
    target=argv[4][0];
    ...
    if(njobs==0){ send_text(resp_fd,"FINAL RESULT: total matches = 0\n"); terminate_all(workers); close(cmd_fd); close(resp_fd); return 0; }

    while(!stop_flag){
        fd_set fds;
        FD_ZERO(&fds); FD_SET(cmd_fd,&fds); maxfd=cmd_fd;
        for(i=0;i<MAX_WORKERS;i++) if(workers[i].alive){ FD_SET(workers[i].from_fd,&fds); if(workers[i].from_fd>maxfd) maxfd=workers[i].from_fd; }
        rc=select(maxfd+1,&fds,NULL,NULL,NULL);
        if(rc<0&&errno!=EINTR){ write_message(2,"dispatcher select failed\n"); break; }

        if(progress_flag){ send_progress(resp_fd,jobs,njobs); progress_flag=0; }
        if(info_flag){ send_info(resp_fd,workers); info_flag=0; }
        if(child_flag){ reap_dead_workers(workers,jobs,njobs); child_flag=0; }

        if(FD_ISSET(cmd_fd,&fds)){
    message_t cmd;
    ssize_t r = read_all(cmd_fd, &cmd, sizeof(cmd));
    ...
    if(cmd.type == CMD_ADD_WORKER && cmd.value > 0){
        ...
        for(i=0;i<MAX_WORKERS;i++) if(workers[i].alive&&FD_ISSET(workers[i].from_fd,&fds)) handle_reply(workers,jobs,njobs,i);
        assign_jobs(workers,jobs,njobs,target);

        if(all_jobs_done(jobs,njobs)){
            char buf[128];
            snprintf(buf,sizeof(buf),"FINAL RESULT: total matches = %d\n",total_matches(jobs,njobs));
            send_text(resp_fd,buf);
            break;
        }
    }
